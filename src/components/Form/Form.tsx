import React, { useState, FC, useMemo, useEffect, KeyboardEvent, ChangeEvent } from "react";
...
const Form: FC = () => {
    ...
    useEffect(() => {
        ...
        //Lepiej podawać tylko to co konkretnie chcemy wyłączyć w następnej linii
    ...
    // Dodawaj trochę "powietrza" między funkcjami
    return (
        <form className="text-white rounded-md bg-white overflow-hidden">
            ...
            <div className="absolute m-auto right-0 w-1/4 bg-white left-0 top-50 w-300 mt-2 rounded-md overflow-hidden">
                ...
                {inputValue ? suggestions?.map((suggestion, i) => {
                ...
                {!suggestions.length && !isSuggestionsLoading && inputValue.length >= 3 ? (
                    <div className="text-black text-base p-3 cursor-pointer">
                        There is no hint!
                    </div>
                ) : null}
